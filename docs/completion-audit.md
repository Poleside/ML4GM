...
evidence. “Complete locally” does not mean “merged” or “released.” The public
preparation branch and its CI are verified; publication to the default branch,
the release decision, and form submission remain pending.

The package, workflow, application drafts, governance, and audit documents were
verified together as one working tree on the audit date. This subsequent
public-evidence refresh is locally verified from PR #1's published predecessor;
after it is committed and pushed, repeat every check against the exact committed
public revision before merge or release.
...
| 4 | CI validates linting, tests, packaging, dependencies, and secrets. | `.github/workflows/ci.yml` defines quality, package, LightGBM, PyTorch, blocking `dependency-audit` with `pip-audit`, and full-history `secret-scan` with Gitleaks. Draft PR [#1](https://github.com/Poleside/ML4GM/pull/1) passed all seven jobs in both push run `29551062270` and pull-request run `29551097543` for commit `82cf975d0530cdc2d0e9020022e1bd37a0b575b0`. | **Complete on public preparation branch** | Require the same checks on the exact merge revision. |
...
| 9 | Public repository state supports every factual claim in both applications. | Draft claims are repository-tested and limited answers are under 500 characters. The public preparation branch, draft PR, and green CI are now verifiable; default `master` still reports no detected license because it has not merged the preparation branch. | **Pending default-branch publication** | Merge only after the remaining release gates close, then refresh metrics/license/permissions and re-review every claim. |
...
- Secret review: heuristic current-tree and history pattern searches found no
  ...
  Blocking `pip-audit` and full-history Gitleaks jobs use minimal permissions
  and passed in both public CI runs for draft PR #1.

## Release decision

Maintainer review authorized the public preparation branch and draft PR #1,
whose CI is green. Do not merge to `master`, create or push `v0.1.0`, publish a
release, add a dated 0.1.0 changelog heading, or submit either form until its
remaining gates close. Merge and release require maintainer approval; form
action additionally requires the approved personal details and explicit user
authorization.
