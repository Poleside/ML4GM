...
def test_governance_facts_and_policies_are_explicit() -> None:
    ...
    assert "no public release has yet been published" in " ".join(security.split())
    assert "If private vulnerability reporting is enabled" in security
    assert "public contact method on their GitHub profile" in " ".join(security.split())
...
def test_citation_metadata_fields_match_the_unreleased_project() -> None:
...
def test_changelog_does_not_claim_an_unpublished_release() -> None:
    text = _read("CHANGELOG.md")
    assert "Preparing 0.1.0" in text
    assert "## [0.1.0]" not in text
    assert "/releases/tag/v0.1.0" not in text
...
def test_temporal_protocol_links_to_leakage_regressions() -> None:
    protocol = _read("docs/benchmark-protocol.md")
    regression_tests = _read("tests/integration/test_sequence_models.py")
    assert "../tests/integration/test_sequence_models.py" in protocol
    for regression in (
        "test_temporal_loyo_filters_held_out_year_from_unbalanced_glacier_context",
        "test_temporal_block_filters_entire_held_out_year_group_from_context",
    ):
        assert regression in protocol
        assert f"def {regression}" in regression_tests


def test_readme_quickstart_executes(tmp_path: Path) -> None:
    executable = shutil.which("ml4gm", path=str(Path(sys.executable).parent))
    assert executable is not None, "the dev environment must install the ml4gm console entry point"
